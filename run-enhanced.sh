...
if docker ps | grep -q mongodb; then
    echo -e "${GREEN}✅ MongoDB: 실행 중 (포트 27017)${NC}"
else
    echo -e "${YELLOW}🔄 MongoDB 시작 중...${NC}"
    ...
    sleep 10
    echo -e "${GREEN}✅ MongoDB: 시작됨${NC}"
fi

# Enhanced Backend API 시작
echo -e "${YELLOW}🔄 Enhanced Backend API 시작 중...${NC}"
if [ -f enhanced-api.js ]; then
    node enhanced-api.js > /tmp/enhanced-api.log 2>&1 &
    API_PID=$!
    sleep 5
    ...
    echo -e "${YELLOW}   enhanced-api.js 없음, 기존 API 사용${NC}"
    node aws-api.js > /tmp/aws-api.log 2>&1 &
    sleep 3
...
# Frontend 서버 시작 (포트 3001)
echo -e "${YELLOW}🔄 Frontend 서버 시작 중...${NC}"
python3 -m http.server 3001 > /tmp/frontend-server.log 2>&1 &
FRONTEND_PID=$!
sleep 3
echo -e "${GREEN}✅ Frontend 서버: 시작됨 (포트 3001, PID: $FRONTEND_PID)${NC}"
